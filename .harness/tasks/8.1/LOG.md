...
- Task 8.1 TDD red: `UV_CACHE_DIR=.uv-cache /home/daolts/.local/bin/uv run pytest -q tests/test_build_profile.py -k monorepo` failed because analyzer returned no services for `apps/*`, `services/*`, and `packages/*` fixture paths.
- Implemented workspace service discovery, ignored directory expansion, and workspace marker recording in `RepositoryAnalysis.workspace_markers`.
- Verified Task 8.1 with Acceptance and full pytest: `1 passed, 7 deselected`; `53 passed`.
