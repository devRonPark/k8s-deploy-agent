...
- Task 8.2 TDD red: `UV_CACHE_DIR=.uv-cache /home/daolts/.local/bin/uv run pytest -q tests/test_manifest_plan.py -k port` failed with missing `k8s_deploy_agent.manifest_plan`.
- Added `manifest_plan.py` with `PortCandidate` and `collect_port_candidates()` for Dockerfile EXPOSE, Compose ports/expose/command, runtime command, nginx listen, package scripts, and explicit config ports.
- Added `tests/test_manifest_plan.py` synthetic fixture coverage for Dockerfile/config/runtime, Compose host/container split, Compose expose/command, nginx reverse proxy, Node dev/runtime scripts, and Vite config.
- Verified Task 8.2 with Acceptance and full pytest: `3 passed`; `56 passed`.
- Manual validation against local sample repositories under `/tmp/pytest-of-daolts/pytest-33`: sample-repo, root-fastapi, monorepo fixture, and compose fixture produced expected `PortCandidate` output.
